...
cheeky_mps <- summarise(reject_expenses, Freq = n(), Value = (sum(`Amount Claimed`)))
              

kable(arrange(cheeky_mps, desc(Freq))[1:10,])


ggplot(reject_expenses) + geom_bar(aes(x = `MP's Name`, fill = Category)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
